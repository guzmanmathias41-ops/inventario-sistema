import { categoriaSalidaService, isNameUnique } from '../services/base.js'
import { flashMessage } from '../../partials/modals.js'
import { GRUPOS_CATEGORIA_SALIDA, GRUPOS_CATEGORIA_SALIDA_LABELS, GRUPOS_CATEGORIA_SALIDA_ICONS } from '../models/schema.js'

export async function renderCategoriasSalida(params, container) {
  let categorias = []
  let filtroGrupo = ''

  async function loadData() {
    try {
      categorias = await categoriaSalidaService.getAll()
    } catch (err) {
      console.error('Error cargando categorías de salida:', err)
      categorias = []
    }
  }
  ...
  async function render() {
    ...
    container.innerHTML = `
      <div data-aos="fade-up">
        <header class="mb-6" data-aos="fade-down">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-white">Categorías de Salida</h1>
              <p class="text-dark-300 text-sm mt-1">Personas, motivos y operativas</p>
            </div>
        ...
        <div class="filter-bar" data-aos="fade-up">
          <select id="filtro-grupo" class="select w-48">
            ...
            ${GRUPOS_CATEGORIA_SALIDA.map(g => `
              <option value="${g}" ${filtroGrupo === g ? 'selected' : ''}>${GRUPOS_CATEGORIA_SALIDA_LABELS[g]}</option>
            `).join('')}
          </select>
        ...
        <div class="table-container" data-aos="fade-up">
          ${filtered.length === 0 ? `
            <div class="p-12 text-center text-dark-400">
              <i class="ph ph-sign-out text-5xl mb-3 block"></i>
              <p class="text-lg">No hay categorías de salida</p>
                  ...
                  <tr class="table-row">
                    <td class="px-5 py-3">
                      <div class="flex items-center gap-2">
                        <i class="ph ${GRUPOS_CATEGORIA_SALIDA_ICONS[cat.grupo] || 'ph-sign-out'} text-dark-400"></i>
                        <span class="text-white font-medium">${cat.nombre}</span>
                      </div>
                    </td>
                    <td class="px-5 py-3">
                      <span class="${grupoBadgeClass[cat.grupo] || 'badge-orange'}">${GRUPOS_CATEGORIA_SALIDA_LABELS[cat.grupo] || cat.grupo}</span>
                    </td>
                    <td class="px-5 py-3 text-right">
                      <button data-edit="${cat.id}" class="text-dark-400 hover:text-primary transition-colors mr-3" title="Editar">
                        <i class="ph ph-pencil text-lg"></i>
                      </button>
                      <button data-delete="${cat.id}" class="text-dark-400 hover:text-accent-red transition-colors" title="Eliminar">
      ...
      <div id="modal-cat-salida" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 hidden">
        <div class="bg-dark-800 border border-dark-600 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
          <h3 class="text-lg font-semibold text-white mb-4" id="modal-title">Nueva Categoría de Salida</h3>
          <form id="form-cat-salida">
            <input type="hidden" id="cat-id" value="">
            <div class="mb-4">
              <label class="block text-sm text-dark-300 mb-1">Nombre</label>
              <input type="text" id="cat-nombre" class="input" placeholder="Ej: Mersh" required>
            </div>
            <div class="mb-6">
              <label class="block text-sm text-dark-300 mb-1">Grupo</label>
              <select id="cat-grupo" class="select w-full">
                ${GRUPOS_CATEGORIA_SALIDA.map(g => `
                  <option value="${g}">${GRUPOS_CATEGORIA_SALIDA_LABELS[g]}</option>
                `).join('')}
              </select>
            </div>
  ...
  function bindEvents() {
    ...
    document.getElementById('form-cat-salida')?.addEventListener('submit', handleSubmit)
  ...
  function openModal(cat = null) {
    const modal = document.getElementById('modal-cat-salida')
    ...
    if (cat) {
      title.textContent = 'Editar Categoría de Salida'
      idField.value = cat.id
      nombreField.value = cat.nombre
      grupoField.value = cat.grupo
    } else {
      title.textContent = 'Nueva Categoría de Salida'
      idField.value = ''
      nombreField.value = ''
      grupoField.value = GRUPOS_CATEGORIA_SALIDA[0]
    }
  ...
  function closeModal() {
    document.getElementById('modal-cat-salida')?.classList.add('hidden')
  }
  ...
  async function handleSubmit(e) {
    ...
    const isUnique = await isNameUnique('categorias_salida', nombre, id || null)
    if (!isUnique) {
    ...
    try {
      if (id) {
        await categoriaSalidaService.update(id, { nombre, grupo })
        flashMessage('Categoría actualizada correctamente', 'success')
      } else {
        await categoriaSalidaService.create({ nombre, grupo })
  ...
  async function handleDelete(cat) {
    ...
    try {
      await categoriaSalidaService.remove(cat.id)
